...
@implementation goods_add_LR_cell
...
- (void)loadModel:(id)model{
    ...
    goods_add_LR *m = (goods_add_LR *)model;
    self.label.text = m.title;
    if (m.title1.length > 0){
        self.label1.text = m.title1;
    }

}
@end
@implementation goods_add_LR
- (NSString *)identifier{ return NSStringFromClass([goods_add_LR_cell class]);}
- (NSValue *)size{
    int h = [view_h textSize:self.title1];
    return [NSValue valueWithCGSize:CGSizeMake(1, h)];
}
+ (goods_add_LR *)creat:(NSString *)title title1:(NSString *)title1{
    
    goods_add_LR *test = [[goods_add_LR alloc]init];
    test.title = title;
    test.title1 = title1;
